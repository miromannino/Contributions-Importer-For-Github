...
DEFAULT_JITTER = (
    ((DEFAULT_TIME_RANGE[1] - DEFAULT_TIME_RANGE[0]) / DEFAULT_MAX_COMMITS_PER_DAY) * 60 * 60
) / 2
...
class ImporterFromStats:
    ...
    def process_csv(self, file_path):
        ...
        with open(file_path, "r") as file:
            ...
            for row in rows:
                ...
                scaled_commits = max(0, min(scaled_commits, self.max_commits_per_day))
    ...
    def create_commits(self, date, count):
        ...
        for i in range(count):
            ...
            commit_date = int(
                time.mktime(current_time.timetuple()) + random.randint(0, int(self.jitter))
            )
