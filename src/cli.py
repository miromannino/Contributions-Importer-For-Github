...
def main():
    ...
    stats_parser = subparsers.add_parser(
        "stats", help="Generate commits based on contribution stats."
    )
    stats_parser.add_argument(
        "--csv", required=True, help="Path to the CSV file containing contributions data."
    )
    stats_parser.add_argument("--mock_repo", required=True, help="Path to the mock repository.")
    stats_parser.add_argument(
        "--generator", required=True, help="File type for the generator (e.g., '.ts')."
    )
    stats_parser.add_argument(
        "--max-commits-per-day",
        type=int,
    ...
    stats_parser.add_argument(
        "--author", nargs="+", help="Emails of the author to filter commits by."
    )
    # endregion

    # region 'repo' action
    repo_parser = subparsers.add_parser("repo", help="Import contributions from repositories.")
    repo_parser.add_argument("--repos", nargs="+", help="Paths to the repositories to import from.")
    repo_parser.add_argument("--mock_repo", help="Path to the mock repository.")
    repo_parser.add_argument(
        "--author", nargs="+", help="Emails of the author to filter commits by."
    )
    repo_parser.add_argument(
        "--max-commits-per-day", nargs=2, type=int, help="Max commits per day as a range (min max)."
    )
    repo_parser.add_argument(
        "--commit-max-amount-changes", type=int, help="Max number of changes allowed per commit."
    )
    repo_parser.add_argument(
    ...
    repo_parser.add_argument(
        "--ignored-file-types", nargs="+", help="List of file types to ignore (e.g., .csv, .txt)."
    )
    repo_parser.add_argument(
        "--ignore-before-date", type=str, help="Ignore commits before this date (YYYY-MM-DD)."
    )
    repo_parser.add_argument(
    ...
    repo_parser.add_argument(
        "--keep-commit-messages", action="store_true", help="Keep original commit messages."
    )
    repo_parser.add_argument(
        "--start-from-last", action="store_true", help="Start importing from the last commit."
    )
    # endregion
